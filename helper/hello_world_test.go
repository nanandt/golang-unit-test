...
func BenchmarkTable(b *testing.B) {
	benchmaks := []struct {
		...
		{
			name:    "Muhamad Fatikhurrizky",
			request: "Muhamad Fatikhurrizky",
		},
		{
			name:    "Budi Sudarsono",
			request: "Budi Sudarsono",
		},
	}

	for _, benchmark := range benchmaks {
...
func BenchmarkSub(b *testing.B) {
	b.Run("Ganjar", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			HelloWorld("Ganjar")
		}
	})
	b.Run("Pranowo", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			HelloWorld("Pranowo")
		}
	})
...
func BenchmarkHelloWorldBanyuLangit(b *testing.B) {
	for i := 0; i < b.N; i++ {
		HelloWorld("BanyuLangit")
	}
}
